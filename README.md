...
De uma praia serão armazenados a localização, extensão, quantidade de bóias e quais são os pontos de referência. Da localização serão armazenados o país, estado e cidade. Das bóias serão armazenados a referência do ponto de coleta, latitude e longitude. Das coletas serão armazenados o número, data, hora, parâmetro, frequência de captação, valor e grandeza. De um sensor serão armazenados o número de série, fabricante, ano de fabricação, data de aquisição e data da próxima manutenção. De uma pessoa serão armazenados a localização de origem, nome, email e telefone de contato. 
